 ...
 *
 * RCS: @(#) $Id: IRCWindowsSystemFactory.cpp,v 1.2 2003/11/16 02:04:20 davygrvy Exp $
 ------------------------------------------------------------------------------
 */
...
IRCWindowsSystemFactory::IRCWindowsSystemFactory() {
  ...
  //GetVersionEx(&ovi); *DEPRECATED*

  // handle at run-time what implimentation we should use.
  // add more logic here, if needed.
  osplat = VER_PLATFORM_WIN32_NT; //ovi.dwPlatformId;
};
...
BlankBoxImp *IRCWindowsSystemFactory::MakeBlankBoxImp ()
{
    return new AllWin32BlankBoxImp;
}
