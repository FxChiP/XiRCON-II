 ...
 *
 * RCS: @(#) $Id: ViewBoxImps.cpp,v 1.1 2003/11/14 10:57:46 davygrvy Exp $
 ------------------------------------------------------------------------------
 */
...
{
public:
    RichEditCTCP2Action(HWND reCtrl, std::wstring *line)
	...
	stx.codepage = 1200;		// 1200 = unicode.
...
private:
	...
	::SendMessage(
		hRE, EM_SETTEXTEX,
		reinterpret_cast<WPARAM>(&stx),
		reinterpret_cast<LPARAM>(segment->c_str())
		);
    ...
    virtual int ParseExtension (std::wstring *string, int *offset, int *endoffset)
    {
	int index, start;
	wchar_t ch;
...
AllWin32ViewBoxImp::AllWin32ViewBoxImp
    (HWND parent, CMclQueue<std::wstring *> &echoQ)
    : m_echoQ(echoQ)
{
    CHARFORMAT2 defFmt;
    PARAFORMAT2 paraFmt;
...
//	    CFM_SIZE |
    ...
    strcpy(defFmt.szFaceName, /*"code2001"*/ "Courier New");
